"""3 Conv + 3 Dense test problem on CIFAR-100 for PyTorch DeepOBS."""
...
class cifar100_3c3d(TestProblem):
    """DeepOBS test problem class for a three convolutional and three dense network on CIFAR-100.
    ...
    The network consists of:
        ...
        - 100-unit output layer with softmax (via cross-entropy loss)
        - Cross-entropy loss
    ...
    Attributes:
        dataset: The CIFAR-100 dataset instance.
    ...
    def __init__(self, batch_size, weight_decay=0.002, device=None):
        """Create a new 3C3D test problem on CIFAR-100.
        ...
        """
        super(cifar100_3c3d, self).__init__(batch_size, weight_decay, device)

    def set_up(self):
        """Set up the 3C3D test problem on CIFAR-100."""
        # Create dataset
        self.dataset = CIFAR100(batch_size=self._batch_size)

        # Create model
        self.model = ThreeC3D(num_outputs=100)
    ...
    def _compute_loss(self, outputs, targets, reduction='mean'):
        ...
        Args:
            outputs (torch.Tensor): Model outputs (logits) of shape (batch_size, 100).
            targets (torch.Tensor): Ground truth labels of shape (batch_size,) or
                (batch_size, 100) for one-hot encoded labels.
