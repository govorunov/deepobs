"""A two-layer LSTM for character-level language modelling on Penn Treebank."""
...
class textgen(TestProblem):
    ...
    - batch size ``64``
    - ``30-50`` epochs
    - Adam with a learning rate of approximately 0.001 works
    ...
    Attributes:
        dataset: The DeepOBS data set class for Penn Treebank.
    ...
    def __init__(self, batch_size, weight_decay=None, device=None):
        """Create a new Char RNN test problem instance on Penn Treebank.
    ...
    def set_up(self):
        """Set up the Char RNN test problem instance on Penn Treebank."""
        # Create dataset
        self.dataset = PennTreebank(batch_size=self._batch_size)

        # Create model with dynamic vocab size from dataset
        self.model = CharRNN(
