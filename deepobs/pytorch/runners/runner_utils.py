...
def float2str(x):
    ...
    Formats a float in scientific notation and removes trailing zeros from
    the mantissa.
    ...
    Returns:
        str: Compact string representation (e.g., "1e-03" instead of "1.0000000000e-03").
...
def make_run_name(weight_decay, batch_size, num_epochs, learning_rate,
    ...
    Generates a descriptive folder name containing all hyperparameters and
    a unique file name with timestamp and random seed.
    ...
    Returns:
        tuple: (run_folder_name, file_name) where:
            ...
            - file_name: Name for the output file, consisting of random seed and a
              time stamp.
    """
...
def make_lr_schedule(lr_base, lr_sched_epochs, lr_sched_factors):
    ...
    Examples:
        - ``make_lr_schedule(0.3, [50, 100], [0.1, 0.01])`` yields
          ``{0: 0.3, 50: 0.03, 100: 0.003}``.
        - ``make_lr_schedule(0.3, None, None)`` yields ``{0: 0.3}``.
        - ``make_lr_schedule(0.3, [], [])`` yields ``{0: 0.3}``.

    Args:
        lr_base (float): A base learning rate.
        lr_sched_epochs (list or None): A (possibly empty) list of integers, specifying
            epochs at which to decrease the learning rate.
        lr_sched_factors (list or None): A (possibly empty) list of floats, specifying
            factors by which to decrease the learning rate.

    Returns:
        dict: A dictionary mapping epoch numbers to learning rates.

    Raises:
        TypeError: If only one of lr_sched_epochs or lr_sched_factors is None.
        ValueError: If lr_sched_epochs and lr_sched_factors are not lists of the
            same length.
    """
